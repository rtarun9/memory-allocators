...
u8 *arena_alloc_array(arena_t *arena, size_t num_elements, size_t size_per_element, size_t alignment)
{
    ...
    // alignment.
    // NOTE: This computation should not take size into account, as size will
    // only determine the alignemnt and new offset for future arena allocations.
    u8 *memory_aligned_result = align_memory_address(arena->base_ptr + arena->offset, alignment);
...
int main()
{
    ...
    u8 *default_aligned_base_2 = arena_alloc_default_aligned(&arena, KILOBYTE(15));
    u8 *default_aligned_base_3 = arena_alloc_default_aligned(&arena, KILOBYTE(32));
    u8 *default_aligned_base_4 = arena_alloc_default_aligned(&arena, KILOBYTE(56));
